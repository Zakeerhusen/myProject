...
sap.ui.controller("corelabs.Dashboard", {
	/**
	 * Called when a controller is instantiated and its View controls (if available) are already created. Can be used to modify the View before it is displayed,
		...
		if (valid){
			...
			oModel.attachRequestCompleted(function(oEvent) {
				...
				if ((data.dashboardSearchDataList && data.dashboardSearchDataList.dashboardStatusSearchTypeList)) {
					if (!(data.dashboardSearchDataList.dashboardStatusSearchTypeList instanceof Array)) {
						data.dashboardSearchDataList.dashboardStatusSearchTypeList = [ data.dashboardSearchDataList.dashboardStatusSearchTypeList ];
					}
	...
	validatePayload : function(payload) {
		...
		if (payload.searchCriteria === "BY_REVENUE") {
			if (parseInt(payload.fromYear) > parseInt(payload.toYear)) {
				...
				if (!valid) {
					sap.ui.commons.MessageBox.show("Time period has to be less than 2 years", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
					return false;
				}
			}
		} else {
		...
		if (err) {
			sap.ui.commons.MessageBox.show("'To' date cannot be earlier from 'From' date", sap.ui.commons.MessageBox.Icon.ERROR, "Error");
			return false;
		}
		return true;
	},
			...
			content : [ new sap.ui.layout.Grid( {
				...
				content : [ new sap.m.HBox( {
					...
					items : [ new sap.m.Text( {
						text : type
					}).addStyleClass("popupFont") ]
				}), new sap.m.HBox( {
					alignItems : "Start",
					items : [ new sap.m.Text( {
						text : label
					}).addStyleClass("popupLabel"), new sap.m.Text( {
						text : value
					}).addStyleClass("popupFont") ]
				}), ]
			}) ]
	...
	clear : function() {
		...
		that.getView().byId("userType").clearSelection();
		that.getView().byId("userType1").setValue("All");
		that.getView().byId("userType1").clearSelection();
	...
	initializeDateInputs : function() {
		...
		this.getView().byId("toMon").setSelectedKey(currentMon+1);
		this.getView().byId("fromMon").setSelectedKey(1);

		this.getView().byId("fromMon1").setSelectedKey("k1");
		this.getView().byId("toMon1").setSelectedKey("k4");
	},
...
// onBeforeRendering: function() {
		...
		// },
		/**
		 * Called when the View has been rendered (so its HTML is part of the document). Post-rendering manipulations of the HTML could be done here. This hook
		 * is the same one that SAPUI5 controls get after being rendered.
		 * 
		 * @memberOf corelabs.Dashboard
		 */
		// onAfterRendering: function() {
